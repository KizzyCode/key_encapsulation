...
=== API Overiew
 ...
 - `size_t buf_max_len(char const* fn_name)`: The *maximum* length a buffer will need to store all data for produced
   by `fn_name`
 ...
 - `uint8_t seal_key(uint8_t* der_tag, slice_t* der_payload, slice_t const* key_to_seal, char const* capsule_key_id,
...
`size_t buf_max_len(char const* fn_name)`.
 ...
 * `1`: ENOBUFS – the provided buffer is too small
 * `2`: EACCES – invalid/missing authentication
 * `3`: EPERM – the operation is not permitted
 * `4`: EIO – an I/O-related error occurred
 * `5`: ECANCELED – the operation was canceled by either the caller or the plugin
 * `6`: ETIMEDOUT – the operation timed out